...
import { createNewClaim, getUserClaims, updateUserClaimsStatus } from "../../services/claimService";
import { useAlert } from "../../context/AlertContext";
import { Delete, Edit, SearchRounded, West } from "@mui/icons-material";

function createData(ClaimId, Title, Description, Status) {
  return { ClaimId, Title, Description, Status };
}
...
const OrganisationClaims = () => {
  ...
  <Select
    ...
    onChange={(e) => {
      ...
      if (selectedStatus) {
        setFilteredRows(rows.filter(row => row.change_status === selectedStatus));
    ...
    displayEmpty
    renderValue={(selected) => (selected ? CLAIM_STATUSES[selected] : "Filter by Status")}
  >
      ...
      <Table aria-label="simple table" >
        ...
        <TableBody >
          ...
          filteredRows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => (
            <TableRow  key={row.claim_id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
...
>
  {row.description}
              ...
              <TableCell align="left">
                <Stack direction="row" alignItems="center" gap={6} justifyContent="flex-start">
                  {row.change_status === 'approved' && <Chip variant="outlined" sx={{ width: 100, }} color="success" label="APPROVED" />}
                  {row.change_status === 'pending' && <Chip variant="outlined" sx={{ width: 100,  }} color="warning" label="PENDING" />}
                  {row.change_status === 'rejected' && <Chip variant="outlined" sx={{ width: 100, }} color="error" label="REJECTED" />}
                  <IconButton onClick={()=>handleUpdateModelOpen(row.claim_id,row.change_status)}>
                    <Edit/>
                  </IconButton>
                  <Button variant="outlined" onClick={()=>navigate(`/claims/detail?claim_id=${row.claim_id}`)} >View in Detail</Button>
...
>
  ...
  {Object.keys(CLAIM_STATUSES).map((status) => (
    <MenuItem key={status} value={status}>
      ...
      <Dialog open={isModalClaimOpen} onClose={() => setIsModalClaimOpen(false)}>
    ...
    <MenuItem key={status} value={status}>
      {CLAIM_STATUSES[status]}
        ...
        <DialogActions>
          ...
          <Button   onClick={() => handleSubmitClaim(selectedClaim.claim_id)} color="primary" variant="contained">
            Update
          </Button>
        </DialogActions>
      </Dialog>
...
export default OrganisationClaims;
